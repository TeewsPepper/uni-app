...
import { OnboardingTour } from "./components/Onboarding/OnboardingTour";
// ✅ NUEVO IMPORT
import BreakZone from "./components/Break/BreakZone";
...
function App() {
  ...
  // ✅ NUEVO ESTADO PARA BREAK ZONE
  const [mostrarBreakZone, setMostrarBreakZone] = useState<boolean>(false);
  ...
  // ✅ NUEVA FUNCIÓN PARA ALTERNAR BREAK ZONE
  const toggleBreakZone = useCallback(() => {
    setMostrarBreakZone(prev => !prev);
  }, []);
  ...
  const handleLogout = useCallback(async (): Promise<void> => {
    try {
      ...
      setUserIdentifier("");
      setShowOnboarding(false);
      // ✅ RESETEAR BREAK ZONE AL CERRAR SESIÓN
      setMostrarBreakZone(false);
    }
  }, []);
    ...
    <div className={styles.dashboard}>
      <div id="dashboard-header">
        <DashboardHeader 
          ...
          onToggleBreakZone={toggleBreakZone}
          mostrarBreakZone={mostrarBreakZone}
        />
      </div>

      {mostrarBreakZone ? (
        // ✅ NUEVO: Mostrar Break Zone
        <BreakZone />
      ) : (
        // ✅ Vista normal (TODO LO EXISTENTE)
        <>
          <div id="proximos-eventos">
            <ProximosEventos
              tareas={tareas}
